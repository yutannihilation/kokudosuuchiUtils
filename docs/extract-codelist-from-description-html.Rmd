...
```{r}
codelist %>%
  add_count(source, text) %>%
  filter(n > 2) %>%
  mutate_all(funs(stringr::str_replace_all(., "\\s", ""))) %>%
  knitr::kable()
```

Except for S12, ignore them as they are ambiguous:

```{r}
codelist <- codelist %>%
  filter(!.data$url %in% c("http://nlftp.mlit.go.jp/ksj/gml/codelist/SectionTypeCd_syuto.html", 
...
```{r}
codelist_w_identifier <- left_join(
  mutate(codelist, source = stringr::str_replace(source, "^datalist-", "")),
  mutate(KSJMetadata_description_url, source = basename(url)) %>% select(source, identifier),
  by = "source"
) %>%
...
```{r}
KSJMetadata_codelist <- c(codelist_codes_all,
                          enum_codes_all)
```
