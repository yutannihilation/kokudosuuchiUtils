...
```{r}
indices_tilda <- dplyr::coalesce(stringr::str_detect(excel_data$code, "～"), FALSE)

# extract tilda
excel_data_part_tilda <- excel_data[indices_tilda, ] %>%
...
```{r}
excel_data <- bind_rows(excel_data[!indices_tilda, ],
                        excel_data_part_tilda)
```
...
KSJMetadata_code_year_cols <- html_data %>% 
  filter(stringr::str_detect(.data$code, "(?<=[^X])XXXX$")) %>%
  {
    tibble::tibble(
      ...
      replacement = stringr::str_c(.$name, "\\1年")
    )
  }
...
readr::write_csv(html_data, rprojroot::find_package_root_file("inst/extdata/excel_data.csv"))
```
